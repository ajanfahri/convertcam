...
void incecerceve(int x1,int y1,int x2,int y2, int w_fcolor);
char *stringal(int x1,int y1,int uzunluk,char *legal,int gelen);
int menu(void);
void analiz(void);
int dosya_ac(void);
void kaydet();
void sadelestir(int baslangic);
int dosya_penceresi(char kayit);
int satir_oku(int baslangic);
void sadece_kod(int baslangic);
void kodlar(void);
void bosluk_koy(int baslangic);
void bosluk(void);
void satirno_sil(int baslangic);
void satirno(void);
...
char *menu2[5]={" Sadece Kod Satirlari   ",
                " Satir Numaralarini Sil ",
                " Bosluk Birak           ",
...
{
 if(toplamsatir>21){
 int y,toplamsayfa,sayfa,cursorpos;
...
if(strstr(baslik,"DOSYA")!=NULL){
cerceve_ciz(baslik,2,17,19,23,RED,BLACK,GREEN,RED);
secim=menu_secim(3,18,menu1, BLACK, WHITE);
}else
{
cerceve_ciz(baslik,40,18,65,23,RED,BLACK,GREEN,RED);
secim=menu_secim(41,19,menu2, BLACK, WHITE);
}
...
switch(secim)
             {
             case 1 : sadece_kod(1);satir_oku(1);menu();break;
             case 2 : satirno_sil(1);satir_oku(1);menu();break;
             case 3 : bosluk_koy(1);satir_oku(1);menu();break;
             case 4 : sadelestir(1);satir_oku(1);menu();break;
             case 27 : break;
             default : break;
             }
}
...
cerceve(5,10,70,15,RED);
gotoxy(30,10);
textcolor(LIGHTGREEN);
textbackground(1);
cprintf("   DOSYA KAYDET   ");
textbackground(0);
gotoxy(8,14);
textcolor(RED);
...
textbackground(0);
textcolor(GREEN);
cprintf(" Degisiklikler Kaydedilsin mi ? ");
chr=255;
while(chr!='E'&&chr!='e'&&chr!='h'&&chr!='H'&&chr!=13&&chr!=27){
chr=getch();
switch(chr){case 'E' : kaydet();deger=1;break;
...
char command[120]="copy ";
  ...
  if(strlen(gecici)<1){clrscr();satir_oku(1);}
       ...
       if ((fptrx = fopen(gecici, "rt")) == NULL)
                    ...
                    cprintf("%s Bulunamadi",gecici);getch();clrscr();satir_oku(1);fclose(fptrx);}
  else{fclose(fptrx);
...
fclose(temp);
degisiklik=0;
//strcpy(dosya_ismi,yazilan_isim);
satir_oku(1);
}
...
while(1){
        switch(secim=kbhits())
         {
                   case PG_UP : if(bassatir-21>0){menu();satir_oku(bassatir-21);gotoxy(wherex(),wherey());}break;
                   case PG_DN : if(bassatir+21<toplamsatir){menu();satir_oku(bassatir+21);gotoxy(wherex(),wherey());}break;
                   ...
                   case F1    : DropDownMenu(2,17,19,23," DOSYA ");break;
                   case F2    : replace_penceresi();satir_oku(1);break;
                   case F3    : DropDownMenu(40,18,65,23," ISLEMLER ");break;
                   case F10   : if(degisiklik){
                                if(CloseQuery()){
                                textbackground(BLACK);textcolor(LIGHTGRAY);clrscr();ayar_kaydet();return 0;}}
                                else{return 0;}
                                break;
                   default    :break;
            }
...
{
//char aranan[40]="",konulacak[40]="";
int i,j;
_setcursortype(2);
gettext(5, 10, 70, 20, buffer);
for(i=10;i<=20;i++){
  for(j=5;j<=70;j++){gotoxy(j,i);cprintf(" ");}}
cerceve(5,10,70,20,RED);
gotoxy(30,10);
textcolor(LIGHTGREEN);
textbackground(1);
cprintf("   DEGISTIR   ");
...
cprintf(" TOPLAM SATIR : %li ",toplamsatir);textbackground(0);}
     cerceve(1,1,79,25,RED);
     gotoxy(35,1);
     textcolor(15);
...
fseek(fptrx, 0L, SEEK_SET);
      gotoxy(40,12);textcolor(RED);cprintf("DOSYA DONUSTURULUYOR ! ! !");
      gotoxy(40,13);cprintf("Lutfen Bekleyiniz...");
while(!feof(fptrx)){
     chr=fgetc(fptrx);
      if (chr=='\n')
                   {
...
/*     if(strstr(kod[j],"TC")!=NULL){
       if(strstr(kod[j],"TC_LASER_ON")!=NULL){strcpy(kod[j],"TC_LASER_ON");}else
       if(strstr(kod[j],"TC_LASER_OFF")!=NULL){strcpy(kod[j],"TC_LASER_OFF");}else
       {strcpy(kod[j],"");}
       }else */
...
ilkmi=1;
  for(i=0;i<=10;i++)
  {
      if(strlen(kod[i])>1&&((kod[i][0]=='G'||kod[i][0]=='X'||kod[i][0]=='I'||kod[i][0]=='J'||kod[i][0]=='Y'||kod[i][0]=='Z'||kod[i][0]=='U'||kod[i][0]=='H'||kod[i][0]=='M'||kod[i][0]=='O'||kod[i][0]=='T'||kod[i][0]=='S'||kod[i][0]=='L'||kod[i][0]=='P')
         &&(kod[i][1]>=48&&kod[i][1]<=57))
         ||((kod[i][0]=='I'||kod[i][0]=='J'||kod[i][0]=='X'||kod[i][0]=='Y'||kod[i][0]=='U'||kod[i][0]=='Z')&&kod[i][1]==45&&(kod[i][2]>=48&&kod[i][2]<=57))
...
if(str1!=NULL)
  {
    if(str1[strlen(aranan)]<48||str1[strlen(aranan)]>57)
    {for(i=0;i<strlen(strSatir)-strlen(str1);i++)
    yeniSatir[i]=strSatir[i];
    strcat(yeniSatir,konulacak);
    int sutun;
    sutun=strlen(yeniSatir);
    for(i=(strlen(strSatir)-strlen(str1))+strlen(aranan);i<strlen(strSatir);i++)
    {yeniSatir[sutun]=strSatir[i];sutun++;}
    }
  }
else
{
...
ilkmi=1;
 ...
 for(i=0;i<10;i++)
  {
   if((strlen(kod[i])>1)&&(kod[i][strlen(aranan)]<48||kod[i][strlen(aranan)]>57)&&(strstr(kod[i], aranan)!=NULL))
/*           (
            (kod[i][0]=='G'||kod[i][0]=='X'||kod[i][0]=='I'||kod[i][0]=='J'||kod[i][0]=='Y'||kod[i][0]=='Z'||kod[i][0]=='U'||kod[i][0]=='H'||kod[i][0]=='M'||kod[i][0]=='O'||kod[i][0]=='T'||kod[i][0]=='S'||kod[i][0]=='L'||kod[i][0]=='P')
            &&(kod[i][1]>=48&&kod[i][1]<=57)
            &&(strcmp(kod[i], aranan)==0)
           )
         ||
           (
             (kod[i][0]=='I'||kod[i][0]=='J'||kod[i][0]=='X'||kod[i][0]=='Y'||kod[i][0]=='U'||kod[i][0]=='Z')
             &&kod[i][1]==45&&(kod[i][2]>=48&&kod[i][2]<=57)
           )
         ||
           (kod[i][1]<48&&kod[i][1]>57)||(strstr(kod[i],aranan)!=NULL)
         ) (kod[i][strlen(aranan)+1]<=48&&kod[i][strlen(aranan)+1]>=57)
     )
         ||(strstr(kod[i],"MSG")!=NULL||strstr(kod[i],"TC")!=NULL))
         if(strcmp(kod[i], aranan)==0)*/
         {
         strcpy(kod[i],konulacak);
         }
      ...
      if (chr=='\n')
                   {
                     ...
                     else{degistir(aranan,konulacak);}
                     strcat(yeniSatir,"\n");
...
char *stringal(int x1,int y1,int uzunluk,char *legal,int gelen)
{
	...
	char al=0,xcursor[]=".";//€
       ...
       else if(gelen==3){strcpy(symtab,file_name);al=strlen(file_name);gotoxy(x1+al,y1);}
do{
		k=kbhits();
		if(k){
    if(k==DOWN){if(gelen==1){secim=history(25,13,63,19,gelen);gotoxy(8,12);cprintf("ARANACAK       : [.....................................] %c",31);
                             gotoxy(26,12);strcpy(symtab,historysec[secim-1]);al=strlen(historysec[secim-1]);cprintf("%s",symtab);gotoxy(x1+al,y1);
                             gotoxy(8,14);cprintf("DEGISTIRILECEK : [.....................................] %c",31);
                             gotoxy(26,14);cprintf("%s",historykon[secim-1]);strcpy(konulacak,historykon[secim-1]);gotoxy(x1+al,y1);
                             }
                if(gelen==2){secim=history(25,15,63,21,gelen);gotoxy(8,14);cprintf("DEGISTIRILECEK : [.....................................] %c",31);
...
char FileNameList[20][13]={""},gecici[13]="",FileExtension[3],isim[7];
...
chdir(kokdizin);
        ...
        switch(chr=kbhits())  {
               ...
               case 9:       if(tab==1){gotoxy(35,5);_setcursortype(2);getch();tab=0;}else{tab=1;}
                             break;//TAB
               case 327:  			 													  	//HOME
                              RealCurPos=0;
                              VisCurPos=0;
...
fseek(fptrx, 0L, SEEK_SET);
      gotoxy(40,12);textcolor(RED);cprintf("DOSYA DONUSTURULUYOR ! ! !");
      gotoxy(40,13);cprintf("Lutfen Bekleyinizi...");
while(!feof(fptrx)){
     chr=fgetc(fptrx);
      if (chr=='\n')
                   {
                     ...
                     kodlar();
                     if(strlen(yeniSatir)>1){
                     fwrite(&yeniSatir, i, 1, temp);fputc('\n', temp);strset(yeniSatir,'\0');}}
                     strset(strSatir,'\0');i=0;satir2++;
                 }
            else
                 {
      ...
      if (chr=='\n')
                   {
                     ...
                     if(strlen(yeniSatir)>1){
                     strcat(yeniSatir,"\n");
                     fwrite(&yeniSatir, strlen(yeniSatir), 1, temp);
                     strset(yeniSatir,'\0');}}
                     strset(strSatir,'\0');i=0;satir2++;
                 }
            else
...
/*                 if ((strSatir[i]>=48&&strSatir[i]<=57)//0..9 rakam ise
                   ||(strchr(illegal,strSatir[i-1])!=0&&strchr(illegal,strSatir[i])!=0)//Bir înceki Harf ise
//                   ||(strSatir[i-1]>=65&&strSatir[i-1]<=90&&strchr(illegal,strSatir[i])!=0)//Bir înceki Harf ise
//                   ||(strSatir[i-1]>=97&&strSatir[i-1]<=122&&strchr(illegal,strSatir[i])!=0)//. ise)//Bir înceki Harf ise
//                   ||(strSatir[i]!=59)
//                   ||(strSatir[i]==46)//. ise
//                   ||(strSatir[i-1]==95)//_ ise
