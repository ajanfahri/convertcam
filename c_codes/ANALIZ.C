...
char sozcuk=1;
unsigned int gtip=0;
void hata_mesaji(char *mesaj);

void hata_mesaji(char *mesaj)
{
char hata_buf[1096];
gettext(20, 11, 60, 13, hata_buf);
_setcursortype(0);
//textbackground(0);
cerceve(20,11,60,13,RED,0);
for(int i=21;i<60;++i){gotoxy(i,12);cprintf(" ");}
gotoxy(23,12);textcolor(LIGHTGREEN);
cprintf("%s",mesaj);
getch();
puttext(20, 11, 60, 13, hata_buf);
...
void convert_ijk(){
double xmer=0,ymer=0,startt=0;//RADIUS=,Xs=0,Ys=0,Zs=0,Xe=0,Ye=0,Ze=0,I,J;
dxfconvert oge;
...
oge.yn2=end.y;
oge.zn2=0;
merkez(oge,&xmer,&ymer,&startt,&startt);
I=J=0;
I=xmer-oge.xn1;
J=ymer-oge.yn1;
                  }
...
{
cerceve_ciz(baslik,34,18,59,23,RED,BLACK,GREEN,RED);
secim=menu_secim(35,19,menu2, BLACK, WHITE);
}
for(i=y1;i<=y2;i++){
for(j=x1;j<=x2;j++){gotoxy(j,i);cprintf(" ");}}
...
switch(secim)
             {
             case 1 : sadelestir(0);satir_oku(1);menu();break;
             case 2 : sadelestir(1);satir_oku(bassatir);menu();break;
             case 3 : sadelestir(2);satir_oku(bassatir);menu();break;
             case 4 : sadelestir(0);sadelestir(3);satir_oku(1);menu();break;
             case 27: break;
             default: break;
             }
}
...
/*  gotoxy(19,9); printf("--------------------");
    gotoxy(19,12);printf("--------------------");
    gotoxy(20,10);printf("   AJAN  CONVERT  ");
    gotoxy(20,11);printf(" Version   01-2005"); //18//feb//2003 version update
delay(1000);
clrscr();   */
ayar_oku();
getcwd(kokdizin,80);
        ...
        switch(secim=kbhits())
         {
                   ...
                   case F3    : DropDownMenu(34,18,59,23," ISLEMLER ");break;
...
//----------------------------- SADELESTIRME -------------------------------//
void sadelestir(int convert_type)// 0 - Sadece Kod // 1 - Satir no sil // 2 - Bosluk birak // 3 - Convert to Ajan
{
int satir2=1,i=0;
char chr;
...
if(strlen(dosya_ismi)<1)
                    {
                    char hata[40];
                    strcpy(hata," F1 Tusuna Basarak Dosya Acmalisiniz.");
                    hata_mesaji(&hata[0]);
                    }
else {
...
char end_var=0;
  for(i=0;i<=20;i++)
  {
     if(strlen(kod[i])>1&&((kod[i][0]=='F'||kod[i][0]=='G'||kod[i][0]=='N'||
            kod[i][0]=='X'||kod[i][0]=='I'||kod[i][0]=='J'||kod[i][0]=='Y'||
        ...
        ||(strstr(kod[i],"MSG")!=NULL))
      {
          if((parameters.satirno_yaz==1&&kod[i][0]!='N')||(parameters.satirno_yaz==0))
          {
                      ...
                      if(kod[i][0]=='X'||kod[i][0]=='Y'||kod[i][0]=='Z'||kod[i][0]=='R'||kod[i][0]=='I'||kod[i][0]=='J')
                      {
                              ...
                              if(kod[i][0]=='X'){if(gtip==0){start.x=atof(&kod[i][1]);}
                              else{end.x=atof(&kod[i][1]);end_var=1;}}
                              else if(kod[i][0]=='Y'){if(gtip==0){start.y=atof(&kod[i][1]);}else{end.y=atof(&kod[i][1]);end_var=1;}}
                              else if(kod[i][0]=='Z'){if(gtip==0){start.z=atof(&kod[i][1]);}else{end.z=atof(&kod[i][1]);end_var=1;}}
    ...
    if(RADYUS!=0&&(strstr(strSatir,"G02")!=NULL||strstr(strSatir,"G03")!=NULL||strstr(strSatir,"G2")!=NULL||strstr(strSatir,"G3")!=NULL)){
                  convert_ijk();RADYUS=0;
                    ...
                    start.x=end.x;start.y=end.y;start.z=end.z;gtip=0;
                   end.x=0;end.y=0;end.z=0;end_var=0;
                 }
}//end of analiz()
...
char yedek_kod[78]="";
 ...
 str1=strstr(kod[j],aranan);
   if(str1!=NULL)
         {
          for(i=0;i<strlen(kod[i])-strlen(str1);i++)
